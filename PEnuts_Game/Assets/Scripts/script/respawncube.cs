...
public class respawncube : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (v.magnitude <= 0.1 &&  (transform.rotation.eulerAngles.y > 5 || transform.rotation.eulerAngles.y < -5 || transform.rotation.eulerAngles.z > 5 || transform.rotation.eulerAngles.z < -5)  )
		{
